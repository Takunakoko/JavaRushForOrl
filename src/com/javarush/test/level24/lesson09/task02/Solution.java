    ...
    public static void sort(List<Stock> list) {
        Collections.sort(list, new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                int resultName = ((String) stock1.get("name")).compareTo((String) stock2.get("name"));
                if (resultName != 0)
                    return resultName;
                else{
                        int resultDate = ((Date) stock1.get("date")).compareTo((Date) stock2.get("date"));
                        if (resultDate != 0)
                            return resultDate * (-1);
                        else
                        {
                            ...
                            if (stock1.containsKey("change"))
                                Solary1 = (Double) stock1.get("change");
                            else
                                Solary1 = (Double) stock1.get("last") - (Double) stock1.get("open");

                            if (stock2.containsKey("change"))
                                Solary2 = (Double) stock2.get("change");
                            else
                                Solary2 = (Double) stock2.get("last") - (Double) stock2.get("open");

                            int resultChange = Solary1.compareTo(Solary2);  //здесь была ошибка
                            if (resultChange != 0)
                                return resultChange * (-1);
                            else
                        return 0;
                        }
                }
