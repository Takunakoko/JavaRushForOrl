...
1.3. имя трэда должно иметь шаблон "GN-pool-A-thread-threadNum", где
GN - это имя группы,
A - это номер фабрики инкрементируется в пределах класса начиная с 1, используйте AtomicInteger
threadNum - номер треда инкрементируется в пределах конкретной фабрики начиная с 1, используйте AtomicInteger
...
public class Solution {
    ...
    private static class AmigoThreadFactory implements ThreadFactory
    {
        ...
        {
             FactoryCount = A.getAndIncrement();
        }
        @Override
        public Thread newThread(Runnable r)
        {
            ...
            thread.setName(String.format("%s-pool-%s-thread-%s", GN, FactoryCount, threadNum.getAndIncrement()));
            return thread;
        }
    }
}
