...
public class Solution
{
    ...
    public static void cleanAllApartaments(List<Apartament> apartaments)
    {
        ...
        for (Apartament obj : apartaments)
        {
            if (obj instanceof Apt1Room)
            {
                ((Apt1Room) obj).clean1Room();
            }
            if (obj instanceof Apt2Room)
            {
                ((Apt2Room) obj).clean2Rooms();
            }
            if (obj instanceof Apt3Room)
                ((Apt3Room) obj).clean3Rooms();
        }
    }
