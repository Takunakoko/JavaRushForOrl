...
public class Solution {
    ...
    public String getPartOfString(String string, String threadName) {
        ...
        try
        {
            ...
            int lastChar = string.lastIndexOf("\t");

            showString = string.substring(firstChar, lastChar);
        } catch (Exception e){
            if (Solution.FIRST_THREAD_NAME.equals(threadName))
            {
                throw new TooShortStringFirstThreadException(e);
            } else                          //Добавляем в класс родительский коснтуктор
            if (Solution.SECOND_THREAD_NAME.equals(threadName))
            {                               //Добавляем в класс родительский коснтуктор
